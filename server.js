...
// Кольори "хто на зміні" беремо з акаунта автора запису (Налаштування →
// Акаунти → колір), а не з фіксованої константи.
...
app.get('/api/export/:year/:month', auth, async (req, res) => {
  try {
    ...
    const nameToId = new Map(members.map(mm => [String(mm.name || '').trim().toLowerCase(), mm.id]));

    // day -> Map(authorKey -> count) — хто скільки записів зробив того дня
    const dayAuthors = new Map();
    function trackAuthor(date, by) {
      if (!date || date.slice(0, 7) !== monthKey || !by) return;
      const day = parseInt(date.slice(8, 10), 10);
      if (!dayAuthors.has(day)) dayAuthors.set(day, new Map());
      const m = dayAuthors.get(day);
      m.set(by, (m.get(by) || 0) + 1);
    }

    function addTime(memberId, date, time) {
      if (!memberId || !date || date.slice(0, 7) !== monthKey || !time) return;
      const day = parseInt(date.slice(8, 10), 10);
      const entry = getEntry(memberId);
      entry[day] = entry[day] || {};
      if (!entry[day].time) entry[day].time = time.replace(':', ' ');
    }
    function addAmount(memberId, date, amount) {
      if (!memberId || !date || date.slice(0, 7) !== monthKey || !amount) return;
      const day = parseInt(date.slice(8, 10), 10);
      const entry = getEntry(memberId);
      entry[day] = entry[day] || {};
      entry[day].amount = (entry[day].amount || 0) + amount;
    }

    // Позначки в колонці "опл/+": зелений — старт абонемента, червоний —
    // кінець абонемента, синій — початок/кінець заморозки.
    const MARK_GREEN = 'FF00FF00', MARK_RED = 'FFFF0000', MARK_BLUE = 'FF0000FF';
    function markPayCell(memberId, date, color) {
      if (!memberId || !date || date.slice(0, 7) !== monthKey) return;
      const day = parseInt(date.slice(8, 10), 10);
      const entry = getEntry(memberId);
      entry[day] = entry[day] || {};
      entry[day].mark = color;
    }

    // 1) Відвідування (час приходу) — з усіх абонементів (клубні + тренерські,
    // включно з тими, що списались під час спліту), а також позначки
    // старту/кінця абонемента і заморозки (тільки для клубних, не тренерських)
    abons.forEach(ab => {
      if (!ab.memberId) return;
      (ab.visits || []).forEach(v => {
        addTime(ab.memberId, v.date, v.time);
        if (v.by) trackAuthor(v.date, v.by);
      });
      if (ab.type !== 'trainer' && !ab.deleted) {
        if (ab.type === 'month' && ab.startDate) markPayCell(ab.memberId, ab.startDate, MARK_GREEN);
        if (ab.type === 'month' && ab.endDate) markPayCell(ab.memberId, ab.endDate, MARK_RED);
        (ab.freezeLog || []).forEach(f => {
          if (f.from) markPayCell(ab.memberId, f.from, MARK_BLUE);
          if (f.to) markPayCell(ab.memberId, f.to, MARK_BLUE);
        });
        if (ab.frozen && ab.freezeStart) markPayCell(ab.memberId, ab.freezeStart, MARK_BLUE);
      }
    });

    // 2) Гроші — рахуємо ТІЛЬКИ те, що реально йде в касу залу:
    //    - kind:'abon'         — повна сума (клубний платіж клієнта)
    //    - kind:'session'      — тільки hallEarning (частка залу), не заробіток тренера
    //    - kind:'trainer_abon' — amount вже є часткою залу
    //    Непідтверджені (hallMethod/method == null) поки не рахуємо.
    payments.forEach(p => {
      if (!p.date || p.date.slice(0, 7) !== monthKey) return;

      if (p.kind === 'abon') {
        if (!p.memberId || p.method == null) return;
        addAmount(p.memberId, p.date, p.amount || 0);
        if (p.by) trackAuthor(p.date, p.by);
        return;
      }

      if (p.kind === 'trainer_abon') {
        if (!p.memberId || p.method == null) return;
        addAmount(p.memberId, p.date, p.amount || 0);
        addTime(p.memberId, p.date, p.time);
        if (p.by) trackAuthor(p.date, p.by);
        return;
      }

      if (p.kind === 'session') {
        if (p.hallMethod == null) return;
        if (p.by) trackAuthor(p.date, p.by);
        if (p.sessionType === 'split' && Array.isArray(p.splitDetails)) {
          const payingCount = p.splitDetails.filter(d => d.paid && !d.isTrainer).length;
          const perPersonHall = payingCount > 0 ? Math.round((p.hallEarning || 0) / payingCount) : 0;
          p.splitDetails.forEach(d => {
            if (d.isTrainer) return;
            const mid = d.id || nameToId.get(String(d.name || '').trim().toLowerCase());
            if (!mid) return;
            addTime(mid, p.date, p.time);
            if (d.paid && perPersonHall) addAmount(mid, p.date, perPersonHall);
          });
        } else {
          if (!p.memberId) return;
          addTime(p.memberId, p.date, p.time);
          addAmount(p.memberId, p.date, p.hallEarning || 0);
        }
      }
    });
    ...
    // всі клієнти з бази, в алфавітному порядку (як у зразку)
    const activeMembers = members.slice().sort((x, y) =>
      String(x.name || '').localeCompare(String(y.name || ''), 'uk')
    );
    ...
    activeMembers.forEach((mem, i) => {
      ...
      for (let d = 1; d <= daysInMonth; d++) {
        ...
        let payCell = null;
        if (dayData.amount) {
          payCell = ws.getCell(row, colP);
          payCell.value = dayData.amount;
          payCell.font = { name: 'Calibri' };
          ...
          // був візит, але окремої оплати того дня немає — покрито абонементом
          payCell = ws.getCell(row, colP);
          payCell.value = '*';
          payCell.font = { name: 'Calibri' };
          payCell.alignment = { horizontal: 'center', vertical: 'middle' };
        }
        // Позначка старту/кінця абонемента чи заморозки — навіть якщо в
        // клітинці немає ні суми, ні "*"
        if (dayData.mark) {
          if (!payCell) {
            payCell = ws.getCell(row, colP);
            payCell.alignment = { horizontal: 'center', vertical: 'middle' };
          }
          payCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: dayData.mark } };
        }
      }
